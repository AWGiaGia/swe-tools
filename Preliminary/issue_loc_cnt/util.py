...
def load_ground_truth(swe_bench_like_dataset_path, local_repo_path, target_repo):
    ...
    for item in swe_bench_like_dataset['test']:
        ...
        if target_repo not in instance_id:
            print(f"Warning: {instance_id} not in {target_repo}. So skip it")
            continue
