    ...
    def add_logic_path(self, data_info, commit_sha, pr_number):
        ...
        def check_commit_exists(repo, commit_hash):
            try:
                ...
                # 添加详细的错误信息
                print(f"   Repo: {repo}")
                print(f"❌ Commit check failed for {commit_hash}")
...
def load_ground_truth(swe_bench_like_dataset_path, local_repo_path, repo_name_prefix):
    ...
    for item in swe_bench_like_dataset['test']:
        if not item['repo'].startswith(repo_name_prefix):
            continue
